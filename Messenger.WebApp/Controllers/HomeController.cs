...
namespace Messenger.WebApp.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        #region دریافت پیامهای چت خصوصی

        /// <summary>
        /// دریافت پیامهای اولیه چت خصوصی (مشابه GetChatMessages)
        /// </summary>
        /// <param name="chatKey">کلید چت (systemchat_ یا private_)</param>
        /// <param name="pageNumber">شماره صفحه</param>
        /// <param name="pageSize">تعداد پیام در هر صفحه</param>
        /// <returns></returns>
        public async Task<IActionResult> GetPrivateChatMessages(string chatKey, int pageNumber = 1, int pageSize = 50)
        {
            try
            {
                var userId = HttpContext.User.Claims.FirstOrDefault(c => c.Type == "UserId")?.Value;
                ...
                var messages = await _messageService.GetPrivateChatMessagesAsync(
                    chatKey,
                    pageNumber,
                    pageSize,
                    messageId: 0,
                ...
                {
                    // چت سیستمی - استفاده از نام ثابت یا از appsettings
                    chatName = "ایران اروپا";
                }
                else if (chatKey.StartsWith("private_"))
                {
                    // چت خصوصی - استخراج userId مقابل از chatKey
                    // فرمت: private_{userId1}_{userId2}
                    var parts = chatKey.Split('_');
                    if (parts.Length >= 3)
                    {
                        var otherUserIdStr = parts[1] == userId ? parts[2] : parts[1];
                ...
                // تنظیم ViewData
                ViewData["chatKey"] = chatKey;
                ViewData["chatName"] = chatName;
                ViewData["baseUrl"] = _baseUrl;
                ViewData["chatType"] = "Private";

                return PartialView("_ChatMessageBody", messages);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error in GetPrivateChatMessages for chatKey {chatKey}", chatKey);
                return StatusCode(500, "Error loading private chat messages");
            }
        }

        /// <summary>
        /// دریافت پیامهای قدیمی‌تر چت خصوصی (مشابه GetOlderMessages)
        /// </summary>
        /// <param name="chatKey">کلید چت</param>
        /// <param name="messageId">آیدی آخرین پیام لود شده</param>
        /// <param name="pageSize">تعداد پیام برای دریافت</param>
        /// <returns></returns>
        public async Task<IActionResult> GetOlderPrivateChatMessages(string chatKey, long messageId, int pageSize = 50)
        {
            try
            {
                var userId = HttpContext.User.Claims.FirstOrDefault(c => c.Type == "UserId")?.Value;
                ...
                _logger.LogInformation($"GetOlderPrivateChatMessages: Loading messages before ID {messageId} for chatKey {chatKey}");
            ...
            {
                _logger.LogError(ex, "Error in GetOlderPrivateChatMessages for chatKey {chatKey}", chatKey);
                return Json(new { success = false, message = "Error loading older private chat messages" });
            }
        }

        /// <summary>
        /// دریافت پیامهای جدید‌تر چت خصوصی (مشابه GetNewerMessages)
        /// </summary>
        /// <param name="chatKey">کلید چت</param>
        /// <param name="messageId">آیدی آخرین پیام نمایش داده شده</param>
        /// <param name="pageSize">تعداد پیام برای دریافت</param>
        /// <returns></returns>
        public async Task<IActionResult> GetNewerPrivateChatMessages(string chatKey, long messageId, int pageSize = 50)
        {
            try
            {
                var userId = HttpContext.User.Claims.FirstOrDefault(c => c.Type == "UserId")?.Value;
                ...
                _logger.LogInformation($"GetNewerPrivateChatMessages: Loading messages after ID {messageId} for chatKey {chatKey}");

                // دریافت پیامهای جدید‌تر
                var messages = await _messageService.GetPrivateChatMessagesAsync(
                    chatKey,
                    pageNumber: 1,
                    pageSize: pageSize,
            ...
            {
                _logger.LogError(ex, "Error in GetNewerPrivateChatMessages for chatKey {chatKey}", chatKey);
                return Json(new { success = false, message = "Error loading newer private chat messages" });
            }
        }
