    ...
    public class MessageService : IMessageService
    {
        ...
        public async Task<IEnumerable<MessageDto>> GetPrivateMessagesAsync(long userId1, long userId2, int pageNumber, int pageSize) // Assuming userId1 is currentUserId
        {
            ...
            var messages = await _context.Messages
                ...
                .Where(m => ((m.SenderUserId == userId1 && m.OwnerId == userId2) ||
                             (m.SenderUserId == userId2 && m.OwnerId == userId1)) && 
                            m.MessageType == (byte)EnumMessageType.Private && !m.IsHidden)
            ...
            long messageId, bool loadOlder = false, bool loadBothDirections = false)
        {
            try
            {
                // Access check - skip for Private chats
                if (chatType != "Private")
                {
        ...
        /// <returns></returns>
        private IQueryable<Message> GetMessagesQuery(long chatId, EnumMessageType messageType)
        {
            return messageType switch
            {
                EnumMessageType.Group => _context.Messages
                    .AsNoTracking()
                    .Where(m => m.OwnerId == chatId && !m.IsHidden && 
                                (m.MessageType == (byte)EnumMessageType.Group || m.MessageType == 3))
                    .Include(m => m.MessageTexts)
                    .Include(m => m.SenderUser)
                ...
                EnumMessageType.Channel => _context.Messages
                    .AsNoTracking()
                    .Where(m => m.OwnerId == chatId && !m.IsHidden && 
                                (m.MessageType == (byte)EnumMessageType.Channel || m.MessageType == 4))
                    .Include(m => m.MessageTexts)
                    .Include(m => m.SenderUser)
                    .Include(m => m.ReplyMessage).ThenInclude(r => r.MessageTexts)
                    .Include(m => m.ReplyMessage).ThenInclude(r => r.SenderUser)
                ...
                EnumMessageType.Private => _context.Messages
                    .AsNoTracking()
                    .Where(m => m.OwnerId == chatId && !m.IsHidden && 
                                m.MessageType == (byte)EnumMessageType.Private && !m.IsSystemMessage)
                    .Include(m => m.MessageTexts)
                    .Include(m => m.SenderUser)
                    .Include(m => m.ReplyMessage).ThenInclude(r => r.MessageTexts)
                    .Include(m => m.ReplyMessage).ThenInclude(r => r.SenderUser)
        ...
        public async Task<int> CalculateUnreadCountFromSqlAsync(long userId, long targetId, string groupType)
        {
            try
            {
                ...
                // برای چت خصوصی، از OwnerId استفاده میکنیم
        ...
        public async Task<IEnumerable<PrivateChatItemDto>> GetUserPrivateChatsAsync(long userId)
        {
            try
            {
                ...
                foreach (var group in groupedByOtherUser)
                {
                    ...
                    var otherUser = lastMessage.SenderUserId == userId 
                        ? await _context.Users.FindAsync(otherUserId)
                    ...
                    var unreadCount = await _context.Messages
                        .Where(m => m.SenderUserId == otherUserId && m.OwnerId == userId && 
